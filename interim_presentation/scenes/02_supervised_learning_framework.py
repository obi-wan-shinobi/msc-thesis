...
class SupervisedLearningFramework(Slide):
    ...
    def construct(self):
        ...
        framework_block = VGroup(rows, sol_punchline)
        self.play(FadeOut(framework_block, shift=LEFT * 0.2), run_time=0.6)
        self.next_slide()

        solution = VGroup(sol_label, sol_expr)
        solution.arrange(RIGHT, buff=0.45, aligned_edge=UP)

        # Compute a TARGET position WITHOUT moving the original object
        solution_target = solution.copy()
        solution_target.next_to(title, DOWN, buff=0.55).to_edge(LEFT, buff=1.0)

        self.play(
            solution.animate.move_to(solution_target, aligned_edge=LEFT), run_time=1
        )
        self.next_slide()
